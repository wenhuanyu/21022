                ...
                <div class="print-body">
                    ...
                    <div class="print-row" v-if="showBar=printData[0].value"
                        ...
                        <div class="charts-wrap">
                            <div id="myChart_heart" :style="{width: '900px', height: '300px',marginTop:'70px'}"></div>
                        </div>
                    </div>
                    <div class="print-row" style="padding: 30px" v-if="showBar = printData[2].value">
                        <div class="print-row">
                            <div class="table-name">
                                ...
                                <div>
                                    <p class="no-print">
                                        <span class="edit-btn print-none" v-if="isShow" @click="getProposal({title:'结论',type:10,field:'conclusion'})"><i class="el-icon-edit"></i>点击此处修改</span>
                                    </p>
                                </div>
                            </div>
...
<!--                            <div v-html="jielun" class="report-text-content"></div>-->
                            <div v-html="jielun" class="report-text-content" style="font-weight: normal;white-space:pre;" contenteditable="true"></div>
                    ...
                    <div class="print-row" style="padding: 30px" v-if="showBar = printData[3].value">
                        <div class="print-row">
                            <div class="table-name">
                                ...
                                <div>
                                    <p class="no-print">
                                        <span class="edit-btn print-none" v-if="isShow" @click="getProposal({title:'医生建议',type:8,field:'doctor_proposal'})"><i class="el-icon-edit"></i>点击此处修改</span>
                                    </p>
                                </div>
                            </div>
...
export default {
    ...
    data() {
        return {
            ...
            modifyTitle: [{title: '医生建议', type: 8, field: 'doctor_proposal'},
                            {title: '结论', type: 10, field: 'conclusion'}],
            templateText: '',
            isShow: false,
            showBar:'',
            form: '',
    ...
    methods: {
        ...
        changeHeight () {
            let _this = this
            this.$nextTick(() => {
                if (_this.modifyTitle.type == 10) {
                    var textArea = _this.$refs.test
                }
                if (_this.modifyTitle.type == 8) {
        ...
        async getInfo() {
            ...
            await this.$axios.post('api/breath/compare_report', this.$qs.stringify(params)).then(res => {
                let result = res.data
                if (result.code === 1) {
        ...
        drawLine() {
            ...
            for(var i =0;i<this.form.data.length;i++) {
                series.push({
                    ...
                    name: this.$route.query.time[i],
                    type:'line',
                    stack: '总量',
                    data:this.form.data[i].data
                })
            }
            var xAxis = []
