        ...
        <div class="content_top">
            ...
            <el-form class="search-bar" ref="form" :inline="true" :model="form" style="display: flex">
                ...
                <el-select v-if="emotionId" v-model="program_id" placeholder="请选择" class="state_title" @change="emotionChange($event)"
                           clearable @clear="setValueNullprogram">
                    <el-option
                ...
                <el-select v-if="emotionId" v-model="threshold_id" placeholder="请选择" class="state_title" @change="thresholdChange($event)"
                           clearable @clear="setValueNullthreshold">
                    <el-option
                ...
                <el-select v-if="breathId" v-model="breath_id" placeholder="请选择" class="state_title" @change="breathChange($event)"
                           clearable @clear="setValueNullbreath">
                    <el-option
...
<!--                    <div style="display: inline-block;min-width: 100px;" v-if="operationLeft">-->
                    ...
                    <el-button
                        ...
                        size="small"
                    >
                        <img src="../assets/images/jibie.png">
                    </el-button>
<!--                    </div>-->
...
export default {
    ...
    mounted() {
        ...
        if (this.tableType&& this.nutritionType !==1) {
            this.con = true
        }
    ...
    methods: {
        ...
        async copySubmitBreath() {
            ...
            await this.$axios.post('api/breath_policy/copy_level', this.$qs.stringify({
                program_id:this.breath_id,
                breath_id:this.form.id,
            })).then(res => {
                this.setValueNullbreath()
                this.InterfaceList()
            })
        },
        //复制接口
        ...
        async ProgramList() {
            if(this.breathId) {
                await this.$axios.post('api/breath_policy/get_child_emotion_program').then(res => {
                    if (res.data.code == 1) {
                        this.breathList =  res.data.data
                    }
                })
            }
        ...
        //分页  进入对应下一个页面后 返回当前页的时候不会返回到第一页
        getLastPage() {
            if(sessionStorage.getItem('detail')) {
        ...
        check(_name) {
            let show = false
            if (['孕周指导','报告中心','终端监控','系统设置'].includes(this.$store.state.cmenu)){
                return true
            }
        ...
        contrast() {
            if (this.tableType === 2) {
                ...
                if (this.multipleSelection.length > 0 && this.multipleSelection.length <= 10) {
                    this.$router.replace({
                        ...
                        query: {ids: this.multipleSelection, id: this.$route.query.id,time: this.multipleSelection3}
                    })
                    return;
                }
            }
            if (this.tableType == 3) {
                ...
                if (this.multipleSelection.length > 0 && this.multipleSelection.length <= 10) {
                    this.$router.replace({name: "EmotionDetailContrast", query: {ids: this.multipleSelection,time: this.multipleSelection3}})
                    return;
                }
                return;
            }
        ...
        InterfaceList() {
            ...
            if(this.nutritionType == 1) {
                this.form.exam_type = 1
            }
            if(this.emotionId) {
                this.form.emotion_id = this.form.id
            }
            if(this.breathId) {
                this.form.breath_id = this.form.id
            }
